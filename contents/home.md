
我们只能看到前方很短的距离，但那里已有足够多值得去做的事。 -- Alan Turing

#### Contact

Email: weilaikeqi0808@163.com

#### Education
西安电子科技大学 电子信息工程 本科 | 2027届应届毕业生 | 期望职位：人工智能算法工程师、人工智能研发工程师、软件开发工程师


#### Research Interests
人工智能；机器学习；多模态大模型；计算机视觉；领域自适应

Artificial Intelligence, Machine Learning, Multimodal Large Models, Computer Vision, Domain Adaptation

#### Skills
- 熟练掌握Python编程语言，熟悉PyTorch和其他常用的机器学习库。
- 熟悉CNN、Transformers、VAE、DiT等常用深度学习框架，熟悉数据预处理、特征工程、模型训练和评估等机器学习流程，熟悉Linux命令行操作，能够使用Git进行版本控制。
- 熟悉使用PySide6进行图形界面开发。
- 了解C++编程语言。